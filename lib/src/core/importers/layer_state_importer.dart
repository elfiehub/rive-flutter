...
class LayerStateImporter extends ArtboardImportStackObject {
  ...
  bool addBlendAnimation(BlendAnimation blendAnimation) {
    ...
    if (state is BlendState) {
      ...
      for (final transition in state.transitions.whereType<BlendStateTransition>()) {
        if (transition.exitBlendAnimationId >= 0 && transition.exitBlendAnimationId < blendState.animations.length) {
          transition.exitBlendAnimation = blendState.animations[transition.exitBlendAnimationId];
        }
      }
      return true;
    }
