...
class BlendStateDirectInstance extends BlendStateInstance<BlendStateDirect, BlendAnimationDirect> {
  ...
  void advance(double seconds, StateMachineController controller) {
    ...
    for (final animation in animationInstances) {
      if (animation.blendAnimation.blendSource == DirectBlendSource.inputId.index) {
        dynamic inputValue = controller.getInputValue(animation.blendAnimation.inputId);
        var value = (inputValue is double ? inputValue : animation.blendAnimation.input?.value) ?? 0;
